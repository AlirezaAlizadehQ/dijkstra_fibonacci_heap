...
def read_edges_file(input_file):
    """
    ...
    number_of_vertices = int(lines[0]) + 1
    ...
    for k in range(1, len(lines)):
        ...
        graph[int(line_split[1])].append((int(line_split[0]), int(line_split[2])))
    return graph


def dijkstra(my_graph):
    """
    ...
    shortest_from_source[1] = 0
    for j in range(1, nodes_counts):
        node_list[j] = fibonacci_heap.insert(math.inf, j)
